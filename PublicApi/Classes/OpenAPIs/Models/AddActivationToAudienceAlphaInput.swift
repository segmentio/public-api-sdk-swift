...
public struct AddActivationToAudienceAlphaInput: Codable, JSONEncodable, Hashable {
    ...
    /** Determines when an event is sent to the Destination.  Possible values: Audience Entered: Sends an event when a profile or entity enters the audience. Audience Exited: Sends an event when a profile or entity exits the audience. Audience Membership Changed: Sends an event for both entries and exits. This does not apply to entities. Entity Added: Sends an event when an entity is added to the audience. Entity Removed: Sends an event when an entity is removed from the audience. Entity Values Changed: Sends an event when an entity's property values change. All Events: Sends events for all trigger conditions (enters, exits, and entity value changes).  Note that \"Entity Values Changed\" and \"All Events\" require the data freshness track events feature (the enable-data-freshness-track-events-support flag) to be enabled for the Workspace; requests using these types are rejected otherwise.  Note that events are sent for the profile, unless the audience is a Linked Audience. In that case, events are sent for the target entity defined for that audience. */
